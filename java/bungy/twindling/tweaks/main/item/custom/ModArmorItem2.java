package bungy.twindling.tweaks.main.item.custom;
...
public class ModArmorItem2 extends ArmorItem {
    private static final Map<ArmorMaterial, StatusEffect> MATERIAL_TO_EFFECT_MAP =
            (new ImmutableMap.Builder<ArmorMaterial, StatusEffect>())
                    .put(ModArmorMaterial.OBSIDIAN, StatusEffects.SLOWNESS).build();




    public ModArmorItem2(ArmorMaterial material, EquipmentSlot slot, Settings settings) {
    ...
    private void addStatusEffectForMaterial(PlayerEntity player, ArmorMaterial mapArmorMaterial, StatusEffect mapStatusEffect) {
        ...
        if(hasCorrectArmorOn(mapArmorMaterial, player) && !hasPlayerEffect) {
            player.addStatusEffect(new StatusEffectInstance(mapStatusEffect, 1500));
